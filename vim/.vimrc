...
" Отключил, т.к. два раза вставляет строку в timelog.txt
" Мб потому что два раза перечитывает .vimrc
...
set complete=.,w,b,u,t,k                " автодополнять без поиска по включенным файлам, но по текущим открытым файлам и словарям
set dictionary+=spell                   " добавить в словарь автодополнения словарь проверки орфографии
set spelllang=ru,en                     " какие языки проверяем на орфографию
set spell                               " включаем проверку правописания, чтобы подгрузился словарь для использования в автодополнениях
set nospell                             " выключаем проверку правописания, оставляя словарь загруженным в память
...
autocmd FileType proto setlocal textwidth=110 shiftwidth=4
...
" disable on protobufs
"let g:ale_pattern_options = {'\.proto$': {'ale_enabled': 0}}
let g:ale_pattern_options = {'\.kt$': {'ale_enabled': 0}}
